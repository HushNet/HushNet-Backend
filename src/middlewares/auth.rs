...
use crate::{app_state::AppState, repository::device_repository, models::device::Devices};
...
impl FromRequestParts<AppState> for AuthenticatedDevice {
    ...
    async fn from_request_parts(
        ...
        let vk_bytes = b64.decode(ik_b64).map_err(|_| (StatusCode::BAD_REQUEST, "Bad pubkey b64".into()))?;
        let vk_arr: [u8; 32] = vk_bytes.try_into().map_err(|_| (StatusCode::BAD_REQUEST, "Bad pubkey length".into()))?;
        let vk = VerifyingKey::from_bytes(&vk_arr).map_err(|_| (StatusCode::BAD_REQUEST, "Bad pubkey".into()))?;

        // Signed message
        vk.verify(ts.as_bytes(), &sig).map_err(|_| (StatusCode::UNAUTHORIZED, "Signature mismatch".into()))?;
